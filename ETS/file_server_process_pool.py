...
def handle_client_wrapper(conn, addr):
    # Rewrap the socket in a new process
    try:
        # Duplicate the socket file descriptor to pass across processes
        new_sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM, fileno=conn.fileno())
        handle_client(new_sock, addr)
...
class ProcessPoolServer:
    ...
    def start(self):
        ...
        logging.warning(f"ProcessPoolServer listening on {(self.host, self.port)} with pool size {self.max_workers}")
        ...
        with ProcessPoolExecutor(max_workers=self.max_workers) as executor:
            while True:
                try:
                    ...
                    executor.submit(handle_client_wrapper, conn, addr)
...
def main():
    ...
    svr = ProcessPoolServer(host='172.16.16.101', port=10000, max_workers=max_workers)
    svr.start()
